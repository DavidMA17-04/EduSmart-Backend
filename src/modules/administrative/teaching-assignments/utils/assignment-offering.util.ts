...
export function isGuideOnlyAssignment(row: AssignmentOfferingShape): boolean {
  return row.offeringKind == null && row.subjectId == null && row.specialtyId == null;
}
 ...
 */
export function isImpartableTeachingAssignment(row: AssignmentOfferingShape): boolean {
