...
export function actorHasStudentRole(roles: readonly string[]): boolean {
  return roles.some((role) =>
    roleMatches(role, INSTITUTIONAL_ROLE_STUDENT, Role.STUDENT, 'Estudiante', 'STUDENT'),
  );
}
 ...
 */
export function resolveMyScheduleActorKind(roles: readonly string[]): MyScheduleActorKind {
