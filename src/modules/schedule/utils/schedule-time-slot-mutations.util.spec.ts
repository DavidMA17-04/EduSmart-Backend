...
describe('schedule-time-slot-mutations.util', () => {
  ...
  it('5. bordes iguales no se solapan', () => {
    expect(scheduleTimesOverlap('07:00:00', '07:40:00', '07:40:00', '08:20:00')).toBe(false);
  });

  it('overlap real CLASS–CLASS', () => {
    expect(scheduleTimesOverlap('07:00:00', '07:40:00', '07:20:00', '08:00:00')).toBe(true);
  });

  it('assert range start >= end', () => {
    expect(() => assertScheduleTimeSlotRange('08:00:00', '07:00:00')).toThrow(BadRequestException);
    expect(() => assertScheduleTimeSlotRange('08:00:00', '08:00:00')).toThrow(BadRequestException);
  });

  it('23–24. BREAK/LUNCH + lessonNumber inválido', () => {
    expect(() => assertLessonNumberForSlotType(ScheduleSlotType.BREAK, 1)).toThrow(
      BadRequestException,
    );
    expect(() => assertLessonNumberForSlotType(ScheduleSlotType.LUNCH, 2)).toThrow(
      BadRequestException,
    );
  });
  ...
  it('25/29. CLASS lessonNumber permitido o null', () => {
    ...
    expect(assertLessonNumberForSlotType(ScheduleSlotType.CLASS, null)).toBe(null);
  });

  it('38. named lock key global exacta', () => {
    expect(SCHEDULE_TIME_SLOTS_MUTATE_LOCK_KEY).toBe('edusmart:sched:time-slots:mutate');
  });
  ...
  describe('named lock lifecycle helpers', () => {
    ...
    it('30. GET_LOCK success', async () => {
      ...
      await expect(acquireScheduleTimeSlotsMutateLock(queryRunner as never)).resolves.toBe(
        SCHEDULE_TIME_SLOTS_MUTATE_LOCK_KEY,
      );
